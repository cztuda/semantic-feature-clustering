...
class SingleVariableMFun : public ofc::functionLib::SingleVariableFunction {
private:
...
public:
    SingleVariableMFun();
    SingleVariableMFun(mxArray* function_ptr, int output_size);
    ~SingleVariableMFun();
    
    
    virtual bool evaluate(ofc::math::Real * result, const ofc::math::Real* x) const override;
    
    virtual SingleVariableMFun* copy() const override;
    
    static void serialize(const SingleVariableMFun* f, std::ostream& s);    
    static bool unserialize( SingleVariableMFun** f, std::istream& s);    
    std::string serialize() const;    
    static SingleVariableMFun* unserialize(const std::string& s);
...
private:
    ...
    {
        ar & boost::serialization::base_object<ofc::functionLib::SingleVariableFunction>(*this);
...
BOOST_CLASS_EXPORT_KEY(ofc::SingleVariableMFun)
